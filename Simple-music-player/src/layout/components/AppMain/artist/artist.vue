<template>
  <div class="content-section menuBar-mv">
    <div class="m-artist">
      <div class="artist-avatar">
        <img :src="singerinfo.value.picUrl + '?param=220y220'" alt="">
      </div>
      <div class="artist-name">
        <h2>{{ singerinfo.value.name }}</h2>
        <span>入驻艺人</span>
        <p>{{ singerinfo.value.alias[0] }}</p>
      </div>
  ...
  <div class="content-section">
    <div class="content-section-title">
      专辑歌曲
      <!-- <a href="#" style="color: var(--theme-color);">查看全部</a> -->
    </div>
    <ul class="t-title">
    ...
    <ul class="s-list">
      <li class="adobe-product" v-for="item in singersong.value">
        ...
        <span class="status alb"> {{ item.alias[0] }} </span>
        <span class="status"> 03:21</span>
        ...
        <div class="button-wrapper">
          ...
          <!-- NV -->
          <!-- <svg t="1648883775602" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
            p-id="3156" width="40" height="40">
            <path
...
import { getsingerapi } from '@/request/api.js'
import { reactive, onBeforeMount } from 'vue'
import { useRoute } from 'vue-router'
const router = useRoute()

const singerinfo = reactive([])
const singersong = reactive([])
onBeforeMount(() => {
  showsinger()
})
const showsinger = () => {
  getsingerapi({
    id: router.query.sgingid,
    limit: 30,
    offset: (1 - 1) * 30
  }).then(res => {
    singerinfo.value = res.data.artist
    singersong.value = res.data.hotAlbums
